...
OUTPUT_EMB_PATH = os.path.join(OUTPUT_DIR, "xlnet_embeddings.npy")
OUTPUT_MISSING_PATH = os.path.join(OUTPUT_DIR, "missing_words_xlnet.txt")

# Modello XLNet (puoi cambiarlo, es. "xlnet-large-cased")
...
def build_xlnet_embeddings():
    # 1. Carica il dizionario
    print(f"Carico il dizionario da: {VOCAB_PATH}")
    vocab = load_vocab(VOCAB_PATH)
    print(f"  -> {len(vocab)} parole nel dizionario")

    # 2. Carica tokenizer e modello XLNet
    print(f"Carico tokenizer e modello XLNet: {MODEL_NAME}")
    ...
    print(f"  -> Dimensione embedding XLNet: {embedding_dim}")
    ...
    if missing_words:
        print("ATTENZIONE: alcune parole del dizionario sono state mappate solo a <unk> da XLNet.")
        ...
        print("Tutte le parole del dizionario hanno prodotto almeno un token valido in XLNet ✅")

    # 5. Salva la matrice .npy
    np.save(OUTPUT_EMB_PATH, embeddings)
    print(f"Embedding XLNet salvati in: {OUTPUT_EMB_PATH}")
    print(f"Forma matrice: {embeddings.shape}")


if __name__ == "__main__":
    build_xlnet_embeddings()
